...
public class ComponentFlaxField extends StructureVillagePieces.Village {
	public ComponentFlaxField() {}

	public ComponentFlaxField(StructureVillagePieces.Start start, int type, Random rand, StructureBoundingBox box, EnumFacing facing) {
		super(start, type);
	...
	public boolean addComponentParts(World worldIn, Random random, StructureBoundingBox box) {
		...
		for (int i = 1; i <= 7; ++i)
		{
			this.setBlockState(worldIn, FlaxObjects.FLAX_BLOCK.withAge(MathHelper.getInt(random, 0, 3)), 1, 1, i, box);
			this.setBlockState(worldIn, FlaxObjects.FLAX_BLOCK.withAge(MathHelper.getInt(random, 0, 3)), 2, 1, i, box);
			this.setBlockState(worldIn, FlaxObjects.FLAX_BLOCK.withAge(MathHelper.getInt(random, 0, 3)), 4, 1, i, box);
			this.setBlockState(worldIn, FlaxObjects.FLAX_BLOCK.withAge(MathHelper.getInt(random, 0, 3)), 5, 1, i, box);
		}
	...
	public static ComponentFlaxField createPiece(StructureVillagePieces.Start start, List<StructureComponent> pieces, Random rand, int x, int y, int z, EnumFacing facing, int type)
	{
		StructureBoundingBox structureboundingbox = StructureBoundingBox.getComponentToAddBoundingBox(x, y, z, 0, 0, 0, 7, 4, 9, facing);
		return canVillageGoDeeper(structureboundingbox) && StructureComponent.findIntersecting(pieces, structureboundingbox) == null ? new ComponentFlaxField(start, type, rand, structureboundingbox, facing) : null;
	}
}
